...
sales_data[sales_data$Category == "Electronics" & sales_data$Quantity > 7, ]
...
# 2. Classify orders into High / Medium / Low value by using if else.
sales_data$order_level <- ifelse(sales_data$FinalAmount > 50000, "High order",
                                 ifelse(sales_data$FinalAmount > 20000, "Medium order", "Low order"))

sales_data
# 3. Print revenue of each order by using for loop.
for 
(i in 1:nrow(sales_data)) {
...
for(i in 1:nrow(sales_data))
{
  ...
  print(sales_data[i,c("CustomerName","Category")])
}
...
cat("Average discount is ", mean(sales_data$Discount))
# 2.	Orders with discount > 10%.
sales_data[sales_data$Discount > 10, ]

# 3.	Total discount amount.
cat("total discount is ", sum(sales_data$DiscountAmount))
